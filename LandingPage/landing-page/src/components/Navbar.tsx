...
const Navbar = () => {
    ...
    const renderNavLink = (href: string, label: string) => (
        <a
            key={label}
            href={href}
            className="relative group py-1 text-m font-medium text-[#0C2B4E]/100 transition-colors hover:text-[#D96C06] cursor-pointer"
        >
            {label}
    ...
    return (
        <nav className="top-0 w-full py-4 px-6 flex justify-between items-center max-w-full mx-auto z-50 bg-[#F5EFED]/95 backdrop-blur-sm">
            ...
            <div className="hidden md:flex gap-8">
                ...
                {renderNavLink("#our-team", "Our Team")}
            </div>

            {/* Action Buttons */}
            <div className="flex items-center gap-3 md:gap-4">
                {/* REMOVED 'hidden' class here. Added responsive padding (px-3 on mobile, px-5 on desktop) */}
                <a href="/under-construction" className="flex items-center gap-2 bg-[#D96C06] text-white px-3 py-2 md:px-5 md:py-2.5 rounded-full text-xs font-bold hover:bg-[#19579F] transition shadow-lg hover:-translate-y-0.5 active:translate-y-0">
            ...
            {isOpen && (
                <div className="absolute top-full left-0 w-full bg-[#F5EFED] z-[100] shadow-2xl border-t border-[#0C2B4E]/10 flex flex-col gap-0 md:hidden animate-in slide-in-from-top-2 duration-200 z-index-1000">
                    ...
                    <a href="#our-team" className="text-[#0C2B4E] px-6 py-4 hover:bg-[#19579F]/5 hover:text-[#D96C06] font-medium">Contact Us</a>
                </div>
            )}
        </nav>
    );
